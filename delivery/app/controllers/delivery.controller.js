...
/** Add line item — delivery status 1|2 only; same-merchant good; deducts good.stock, adds to in_delivery */
exports.addDeliveryItem = async (req, res) => {
  const deliveryId = parseInt(req.params.deliveryId, 10);
  const good_id = parseInt(req.body.good_id, 10);
  const qty = parseInt(String(req.body.quantity), 10);

  if (!deliveryId || Number.isNaN(deliveryId)) {
    return res.status(400).json({ success: false, message: "Delivery ID is required" });
  }
  if (!good_id || Number.isNaN(good_id)) {
    return res.status(400).json({ success: false, message: "good_id is required" });
  }
  ...
  try {
    ...
    const st = Number(delivery.status);
    // Only «Шинэ» (1) or «Жолоочид» (2) — same rule as UI
    if (![1, 2].includes(st)) {
      await t.rollback();
      return res.status(400).json({
        success: false,
        message: "Зөвхөн «Шинэ» (1) эсвэл «Жолоочид» (2) төлөвт бараа нэмж болно.",
      });
    }

    const good = await Good.findByPk(good_id, { transaction: t });
    if (!good) {
      await t.rollback();
      return res.status(404).json({ success: false, message: "Бараа олдсонгүй" });
    }

    if (Number(good.merchant_id) !== Number(delivery.merchant_id)) {
      await t.rollback();
      return res.status(400).json({
        success: false,
        message: "Зөвхөн энэ хүргэлтийн дэлгүүрийн барааг нэмж болно.",
      });
    }
    ...
    if (currentStock < qty) {
      ...
      return res.status(400).json({
        success: false,
        message: `Агуулахын үлдэгдэл хүрэлцэхгүй. "${good.name}" — үлдэгдэл: ${currentStock}, хүсэлт: ${qty}`,
      });
    }

    const existing = await DeliveryItem.findOne({
      where: { delivery_id: deliveryId, good_id },
      transaction: t,
    });
    ...
    if (existing) {
      ...
      row = existing;
      // Агуулахын үлдэгдэл (stock) → хүргэлтэнд (in_delivery)
      await good.update(
        {
          stock: (good.stock || 0) - qty,
          in_delivery: (good.in_delivery || 0) + qty,
        },
        { transaction: t }
      );
      await db.good_histories.create(
        {
          ...
          amount: qty,
          delivery_id: deliveryId,
          comment: `Хүргэлтэд бараа нэмэгдсэн (${oldQuantity} → ${newQuantity})`,
        },
        { transaction: t }
      );
    } else {
      row = await DeliveryItem.create(
        {
          delivery_id: deliveryId,
          good_id,
          quantity: qty,
      ...
      await db.good_histories.create(
        {
          ...
          amount: qty,
          delivery_id: deliveryId,
          comment: "Хүргэлтэнд бараа нэмэгдсэн",
        },
        { transaction: t }
      );
    }
    ...
    const full = await DeliveryItem.findByPk(row.id, {
      include: [
        {
          model: Good,
          as: "good",
    ...
    res.status(500).json({
      success: false,
      message: err.message || "Error adding delivery item",
    });
  }
};
