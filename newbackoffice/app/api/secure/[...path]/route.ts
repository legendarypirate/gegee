...
// NOTE: Phone, email, address are NOT encrypted - sent as plain text over HTTPS for table view and forms
const SENSITIVE_REQUEST_FIELDS = [
  'password',      // Always encrypted
  // 'phone',      // NOT encrypted - plain text for user table/form
  // 'email',      // NOT encrypted - plain text for user table/form
  ...
  // 'address',    // NOT encrypted - plain text
];

// Fields that should be encrypted in responses
// NOTE: Phone and email are NOT encrypted - displayed as plain text in user table view
const SENSITIVE_RESPONSE_FIELDS = [
  // 'phone',      // NOT encrypted - plain text in user table
  // 'email',      // NOT encrypted - plain text in user table
  'account_number', // Encrypted - sensitive financial data
  'bank',          // Encrypted - sensitive financial data
  'contact_info',  // Can be encrypted if needed
  // 'address',    // NOT encrypted - plain text
...
/** Safely parse JSON from response body to avoid 500 when backend returns HTML or invalid JSON */
async function safeParseJson(response: Response): Promise<{ data: any; ok: boolean }> {
  ...
  try {
    ...
    return {
      data: { success: false, message: response.ok ? 'Invalid response from server' : `Backend error: ${response.status} ${response.statusText}` },
      ok: false,
    };
  }
}
...
export async function GET(
  ...
  try {
    if (!API_URL) {
      console.error('NEXT_PUBLIC_API_URL or API_URL is not set');
      return NextResponse.json(
        { success: false, message: 'API URL is not configured. Set NEXT_PUBLIC_API_URL or API_URL in production.' },
        { status: 500 }
      );
    }
    ...
    // Get token from cookie (or Authorization header for server-side)
    ...
    const backendUrl = `${API_URL.replace(/\/$/, '')}/api/${path}${queryString}`;
    ...
    if (!ok) {
      return NextResponse.json(data, { status: response.status >= 400 ? response.status : 500 });
    }
    ...
    if (data.success && data.data) {
      try {
        ...
        console.warn('Encrypt sensitive fields failed, returning plain data:', encErr);
      }
    }
