...
export const fetchDeliveries = async (
  ...
  const result = await apiGet<{ success: boolean; data: Delivery[]; pagination: DeliveryPagination; message?: string }>(
...
export const fetchDeliveryItems = async (deliveryId: number): Promise<DeliveryItem[]> => {
  const result = await apiGet<{ success: boolean; data: DeliveryItem[]; message?: string }>(
...
export const fetchDeliveryHistory = async (deliveryId: number): Promise<DeliveryHistory[]> => {
  const result = await apiGet<{ success: boolean; data: DeliveryHistory[]; message?: string }>(
...
export const createDelivery = async (payload: CreateDeliveryPayload): Promise<Delivery> => {
  const result = await apiPost<{ success: boolean; data: Delivery; message?: string }>('/delivery', payload);
...
export const updateDelivery = async (
  ...
  const result = await apiPut<{ success: boolean; data: Delivery; message?: string }>(`/delivery/${deliveryId}`, payload);
...
export const deleteDeliveries = async (ids: React.Key[]): Promise<void> => {
  const result = await apiPost<{ success: boolean; message?: string }>('/delivery/delete-multiple', { ids });
...
export const allocateDeliveries = async (driverId: number, deliveryIds: React.Key[]): Promise<void> => {
  const result = await apiPost<{ success: boolean; message?: string }>('/delivery/allocate', {
    driver_id: driverId,
...
export const changeDeliveryStatus = async (
  ...
  const result = await apiPost<{ success: boolean; message?: string }>('/delivery/status', {
    status_id: statusId,
    delivery_ids: deliveryIds,
  });
...
export const updateDeliveryDates = async (
  ...
  const result = await apiPost<{ success: boolean; message?: string }>('/delivery/update-delivery-dates', {
...
export const fetchDrivers = async (): Promise<User[]> => {
  const result = await apiGet<{ success: boolean; data: User[]; message?: string }>('/user/drivers');
...
export const fetchStatuses = async (): Promise<DeliveryStatus[]> => {
  const result = await apiGet<{ success: boolean; data: DeliveryStatus[]; message?: string }>('/status');
...
export const fetchRegions = async (): Promise<Region[]> => {
  const result = await apiGet<{ success: boolean; data: Region[]; message?: string }>('/region');
...
export const fetchProducts = async (merchantId: number): Promise<Array<{ id: string; name: string; stock: number }>> => {
  const result = await apiGet<{ success: boolean; data: any[]; message?: string }>(`/good?merchant_id=${merchantId}`);
...
export const importDeliveries = async (deliveries: any[]): Promise<{ inserted?: number }> => {
  const result = await apiPost<{ success: boolean; inserted?: number; message?: string }>('/delivery/import', { deliveries });
...
export const updateDeliveryItem = async (
  ...
  const result = await apiPut<{ success: boolean; data: DeliveryItem; message?: string }>(
...
export const deleteDeliveryItem = async (
  ...
  const result = await apiDelete<{ success: boolean; message?: string }>(`/delivery/${deliveryId}/items/${itemId}`);
...
/** Add item to delivery — good must belong to delivery's merchant (enforced server-side). */
export const addDeliveryItem = async (
  deliveryId: number,
  payload: { good_id: number; quantity: number }
): Promise<DeliveryItem> => {
  const result = await apiPost<{ success: boolean; data: DeliveryItem; message?: string }>(
    `/delivery/${deliveryId}/items`,
    {
  ...
  throw new Error(result.message || 'Failed to add delivery item');
};
