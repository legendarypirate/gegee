...
function DeliveryPageContent() {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      // Columns (order): №, Дэлгүүр, Барааны нэр, Тоо, Нийт үнэ, Утас, Дэлгэрэнгүй хаяг
      const excelData: any[] = [];
      ...
      selectedRows.forEach((row) => {
        ...
        if (items.length === 0) {
          excelData.push({
            ...
            'Нийт үнэ': row.price,
            Утас: row.phone,
            'Дэлгэрэнгүй хаяг': row.address,
          });
          currentRowIndex++;
        } else {
          items.forEach((item: DeliveryItem, itemIndex: number) => {
            excelData.push({
              '№': itemIndex === 0 ? deliveryNumber : '',
              Дэлгүүр: itemIndex === 0 ? row.merchant?.username ?? '-' : '',
              'Барааны нэр': item.good?.name || 'Unknown',
              Тоо: item.quantity,
              'Нийт үнэ': itemIndex === 0 ? row.price : '',
              Утас: itemIndex === 0 ? row.phone : '',
              'Дэлгэрэнгүй хаяг': itemIndex === 0 ? row.address : '',
            });
            currentRowIndex++;
          });
        }
      ...
      // Merge: №=0, Дэлгүүр=1, Нийт үнэ=4, Утас=5, Дэлгэрэнгүй хаяг=6 (Барааны нэр, Тоо per line)
      const columnsToMerge = [0, 1, 4, 5, 6];
  ...
  return (
    <div className="w-full mt-6 px-4 pb-32">
      ...
      {/* Fixed Bottom Actions: full bar for staff with permission; merchants (role 2) see Excel Export only */}
      {(hasPermission('delivery:excel_import_delivery') || isMerchant) && (
