...
export default function RequestPage() {
  ...
  const handleToggleActive = useCallback(async (user: User, active: boolean) => {
    setTogglingId(user.id);
    try {
      await updateUser(user.id, { is_active: active });
      toast.success(active ? 'Идэвхтэй болголоо' : 'Идэвхгүй болголоо');
      await fetchData();
    } catch (error: any) {
      toast.error(error.message || 'Өөрчлөхөд алдаа гарлаа');
    } finally {
      setTogglingId(null);
    }
  }, []);
